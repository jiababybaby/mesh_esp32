...
static const char *TAG = "JCQ-i2c";

char tvoc_val[20]={0},eco2_val[20]={0};
char lux_val[20]={0};
...
#define I2C1_MASTER_NUM I2C_NUMBER(1) 
...
{
    int i2c_master_port = I2C1_MASTER_NUM;
    i2c_config_t conf;
    conf.mode = I2C_MODE_MASTER;
    conf.sda_io_num = 14;
    conf.sda_pullup_en = GPIO_PULLUP_ENABLE;
    ...
    while (1) {
       ...
       ret=IAQmeasure(&eCO2,&TVOC);
        if (ret == ESP_ERR_TIMEOUT) {
            ...
            memset(tvoc_val,0,sizeof(tvoc_val));
            memset(eco2_val,0,sizeof(eco2_val));
            sprintf(tvoc_val,"%d",TVOC);
            sprintf(eco2_val,"%d",eCO2);
...
void i2c1_test_task(void *arg)
{
    ...
    int cnt = 0;
    MDF_ERROR_ASSERT(i2c1_master_init());
    while (1) {
        MDF_LOGI( "TASK[%d] test cnt: %d", task_idx, cnt++);
        ret = i2c_master_sensor_test(I2C1_MASTER_NUM, &sensor_data);
        if (ret == ESP_ERR_TIMEOUT) {
            MDF_LOGE( "I2C Timeout");
        } else if (ret == ESP_OK) {
            MDF_LOGI("*******************\n");
            ...
            MDF_LOGI("*******************\n");
            MDF_LOGI("data: %d\n", sensor_data);
            memset(lux_val,0,sizeof(lux_val));
            sprintf(lux_val,"%d",sensor_data);
        } else {
            MDF_LOGW( "%s: No ack, sensor not connected...skip...", mdf_err_to_name(ret));
        }
        vTaskDelay(1000  / portTICK_RATE_MS);
