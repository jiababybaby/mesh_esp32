...
static uint8_t raw_adv_data[] = {
    ...
    0x03, 0x03, 0xFF, 0x00,
    /* device name */
    0x0A, 0x09, 'Z', 'N', 'W', 'G', '-', 'A', 'T', 'T', 'S',
    /* Manufacturer Data*/
    0x06, 0xff, 0xff, 0xff, 0, 0, 0};
    ...
    case ESP_GATTS_WRITE_EVT:
        if (!param->write.is_prep)
        {
            ...
            if (heart_rate_handle_table[IDX_CHAR_VAL_A] == param->write.handle)
            {
                ...
                ESP_LOGI(GATTS_TABLE_TAG, "mode: %d CHANNEL %d, ssid %s pwd %s", mode, mesh_channel, ssid, pwd);
